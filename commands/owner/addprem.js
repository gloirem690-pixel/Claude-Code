  ...
  async execute(sock, msg, args) {
    ...
    try {
      ...
      if (!configmanager.premiums.premiumUser[key]) {
        configmanager.premiums.premiumUser[key] = { premium: number };
        configmanager.saveP();
        await sock.sendMessage(remoteJid, { text: `✅ ${participant} est maintenant premium.` });
      } else {
        await sock.sendMessage(remoteJid, { text: `⚠️ ${participant} est déjà premium.` });
      }
    } catch (error) {
